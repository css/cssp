...
var args = process.argv.slice(2),
    opts = args.length ? getOpts(args, [
        '-v', '--version',
        '-h', '--help',
        '-dt', '--transformer', // dump tree after transforming
        '-dp', '--parser', // dump tree after parsing
    ], ['-r', '--rule']) : null,
...
if (!opts || single.contains(['-h', '--help']) || other.length > 1) {
    ...
    if (single.contains(['-dp', '--parser'])) {
        console.log(array2string(tree, 0));
    }

    trans = transformer.matchAll(tree, rule);
    if (single.contains(['-dt', '--transformer'])) {
        console.log(array2string(trans, 0));
    }

    if (!single.contains(['-dp', '--parser']) && !single.contains(['-dt', '-transformer'])) {
        console.log(translator.matchAll(trans, rule));
    }
}
